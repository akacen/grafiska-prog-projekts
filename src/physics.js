...
export class PhysicsController{
	...
	createPlanet(objThree, radius, mass, pos, quat){
		...
		body.mass = mass;
		this.threeObjects.push(objThree);
	...
	animate(delta){
		for(let objThree of this.threeObjects){
			for(let objThree2 of this.threeObjects){
				...
				if (body != body2){
					...
					const r = pos2.length();
					if(r != 0){
						force.op_mul((100 * mass1 * mass2) / (r * r))  //vajadzētu abu obj masu sareizināt. Nutona gravitacijas formula
	...
	getPosition(objPhys){
		...
		transform.__destroy__(); //atminas menedzments
		return tmp;
	}
